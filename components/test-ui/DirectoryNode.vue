<template>
    <div class="flex items-center justify-between gap-2 py-1 hover:bg-gray-50 rounded group/directory">
            <div class="flex items-center gap-2 cursor-pointer select-none flex-1 min-w-0" @click="toggleOpen"
                ...
                <UIcon name="i-heroicons-folder" class="text-secondary w-5 h-5 flex-shrink-0" />
                <UiText color="secondary" weight="semibold">{{ directory.name }}</UiText>
            </div>
        ...
        <ul v-if="isOpen && directory.artifacts && directory.artifacts.length"
            class="ml-7 border-l border-gray-200 pl-4 mt-1">
            <li v-for="artifact in directory.artifacts" :key="artifact.id"
                class="flex items-center justify-between gap-2 mb-1 py-1 hover:bg-gray-50 rounded group/artifact">
                <div class="flex items-center gap-2">
                    <UIcon name="i-heroicons-document" class="text-neutral-500 w-4 h-4" />
...
import { ref } from 'vue'
import type { DirectoryDto } from '~/types/request.types'
defineProps<{ directory: DirectoryDto }>()
