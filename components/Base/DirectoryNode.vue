<template>
    <div class="flex items-center justify-between gap-2 py-1 hover:bg-gray-50 rounded group/directory">
        <div class="flex items-center gap-2 cursor-pointer select-none flex-1 min-w-0" @click="toggleOpen" tabindex="0"
            @keydown.enter.space="toggleOpen">
        ...
        <div class="flex gap-1 opacity-0 group-hover/directory:opacity-100 transition-opacity">
            <UButton size="xs" variant="ghost" color="neutral" icon="i-heroicons-plus"
                @click.stop="$emit('open-create', directory)" />
            <UButton size="xs" variant="ghost" color="neutral" icon="i-heroicons-pencil"
                @click.stop="$emit('open-rename', directory)" />
            <UButton size="xs" variant="ghost" color="error" icon="i-heroicons-trash"
                @click.stop="$emit('open-delete', directory)" />
            <UButton size="xs" variant="ghost" color="neutral" icon="i-heroicons-arrow-up-tray"
                @click.stop="$emit('upload-file', directory)" />
        </div>
    </div>
    <ul v-if="isOpen && directory.artifacts && directory.artifacts.length" class="ml-7 border-l border-muted pl-4 mt-1">
        <li v-for="artifact in directory.artifacts" :key="artifact.id"
            class="gap-2 mb-1 py-1 hover:bg-muted rounded group/artifact">
            <ULink class="flex items-center justify-between gap-2" :to="artifact.path">
                ...
                <div class="opacity-0 group-hover/artifact:opacity-100 transition-opacity">
                    <UButton size="xs" color="error" variant="ghost" icon="i-heroicons-trash"
                        @click.stop="$emit('delete-file', { directory, artifact })" />
                </div>
            </ULink>
        </li>
    </ul>
